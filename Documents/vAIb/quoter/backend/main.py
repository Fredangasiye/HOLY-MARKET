...
async def predict_pricing(features: QuoteFeatures, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def create_quote(quote_data: QuoteCreate, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    # Get AI pricing recommendation
    features = quote_data.features.dict()
    ...
    db_quote = DBQuote(
        ...
        expires_at=datetime.utcnow() + timedelta(days=quote_data.validity_days)
    )
    ...
    quote_dict = {
        ...
        "terms": quote_data.terms,
        "validity_days": quote_data.validity_days,
        "client_info": quote_data.client_info.dict() if quote_data.client_info else {}
    }
...
async def get_quotes(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    quotes = db.query(DBQuote).filter(DBQuote.user_id == current_user.id).all()
    return quotes

@app.get("/quotes/{quote_id}", response_model=QuoteResponse)
async def get_quote(quote_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def update_quote_status(quote_id: int, status: str, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    quote = db.query(DBQuote).filter(DBQuote.id == quote_id, DBQuote.user_id == current_user.id).first()
    if not quote:
        raise HTTPException(status_code=404, detail="Quote not found")
...
async def get_quote_pdf(quote_id: int, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    quote = db.query(DBQuote).filter(DBQuote.id == quote_id, DBQuote.user_id == current_user.id).first()
    if not quote:
        raise HTTPException(status_code=404, detail="Quote not found")
...
async def train_model(training_data: List[TrainingData], db: Session = Depends(get_db)):
    ...
    data = [item.dict() for item in training_data]
